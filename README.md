...
- **DjangoTemplates** : Django와의 데이터 통신이 빠르고 간편함
...
- 구조가 dict로 변했기 때문에 openai를 통해 추가 작업이 필요 없어짐 으로 인해 검색 속도 10배 이상 상승 및 비용 절감
- Elasticsearch의 query에 검색 조건으로 필터링 <br>

```
업종 : 정확하게 맞게

지역 : 단어 포함이면 검색 허용 ex) '경북'으로 검색해도 '경상북도' 검색 가능

공고내용 : 데이터를 살펴보니 공고 내용에 '경상북도 ~~ 무슨 지역 무슨 지원사업 ~~'
          이런 내용이 많아서 지역을 공고 내용에도 필터링 해서
          지역이 '경상북도' 거나 공고 내용에 '경상북도' 포함 시 검색 허용
...
- 1812 ~ 500 번까지 총 1313개의 게시글에서 데이터 추출
- 전체 데이터 변환 후 Elasticsearch에 인덱싱 (6~8시간 예상이라 집가서 하기)
- 지역, 업종 검색 테스트
...
- 예외처리 추가
- 공고 내용 500자에 가깝게 요약 등 디테일한 설명하기
...
- 웹 페이지 디자인 어떤식으로 할지
- DB에 저장해야할 정보가 뭐가 있을지 <br>
  ex) 회원가입을 해야한다면 사용자 ID, PW 등 개인정보 (Oauth 등), 사용자가 입력한 검색(뭐를 주로 검색했는지), 그에따라 어떤 답변을 했는지 등
- 서버는 뭘 쓸지 ex) AWS, Naver Cloud 등
...
- Django 기반으로 웹 개발
- 데이터 통신이 많지 않아서 Django Templates로 프론트엔드 구현(굉장히 빠름, 만들기 쉬움)
- 디자인 제외하고 기능 우선 구현 <br>
  <img src=".\captures\Django_초안.png">

# 2025.04.28 (월)
## 1. 할일

- DB구조  설계
- UI/UX 디자인 설계 (추천페이지, 전문가 자문 구하기, 관리자 페이지)
- Kakao Oauth 연동, 카카오 공유하기
- 구글 애드센스
- AWS 서버 세팅
- 도메인 뭘로 할지
